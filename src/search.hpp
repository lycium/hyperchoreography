...
struct Ctx {                                  // per-thread state
  ...
  const Problem& problem(const Config& cfg, int K) {
    ...
    if (it == probs.end()) { Problem P; P.init(cfg.N, cfg.d, K, 0, 1.0); P.minsep = cfg.minsep; it = probs.emplace(K, std::move(P)).first; }
    return it->second;
  }
...
inline double return_error(const Problem& P, const double* x, NBody<double>& nb) {
  ...
  return chore_residual(nb, pos, vel, 1e-16) / sc;
}
...
inline bool orbit_fit(int N, int d, double alpha, Orbit& O, Ctx& ctx, int Ms_req, int Kout_max) {
  ...
  for (int j = 0; j < N; j++) for (int i = 0; i < Mseg; i++) for (int c = 0; c < d; c++) { S[((size_t)j * Mseg + i) * 2 * d + c] = seg[(size_t)i * 2 * nd + j * d + c]; S[((size_t)j * Mseg + i) * 2 * d + d + c] = seg[(size_t)i * 2 * nd + nd + j * d + c]; }
...
// Morse index and nullity at ≤ K_index modes
inline void morse_index(const Config& cfg, Record& rec, Ctx& ctx) {
  int Ki = std::min(cfg.K_index, rec.h.K); const Problem& P = ctx.problem(cfg, std::max(Ki, cfg.K)); ctx.w.resize(P);
  ...
  double gn = 0; ctx.g.resize(P.n); if (!std::isfinite(action_grad(P, x.data(), ctx.g.data(), ctx.w))) return; for (double v : ctx.g) gn += v * v;
  rec.h.grad_norm = std::sqrt(gn);
  if (hessian(P, x.data(), ctx.H, ctx.w)) { Inertia I = inertia(P.n, ctx.H); rec.h.morse = I.neg; rec.h.nullity = I.zero; }
}
...
inline bool certify(const Config& cfg, const Problem*& P, std::vector<double>& x, const Symmetry& S, Ctx& ctx, Record& rec, std::string& why) {
  ...
  double res = shoot_newton(nb, O.Z, 1e-16, 10, cfg.shoot_tol * sc, -18, -40, ctx.sw) / sc;
  if (!(res <= cfg.shoot_tol * 100)) { why = "shooting did not certify"; return false; }
  if (!orbit_fit(P->N, P->d, 1.0, O, ctx, cfg.Ms, cfg.Kout_max)) { why = "orbit sampling failed"; return false; }
  O.residual = res;
  std::vector<double> sv; int deff = canonical_frame((int)O.modes.size() * 2, P->d, O.coef, sv);
  if (deff < cfg.min_deff) { why = "effective dimension below filter"; return false; }
  Inertia I; double gn = 0;
  ...
  rec.h.minsep = O.minsep; rec.h.Lnorm = O.Lnorm; rec.Lsv = O.Lsv; rec.pca = sv; rec.h.morse = I.neg; rec.h.nullity = I.zero; rec.h.grad_norm = gn; rec.h.ret_err = res;
  return true;
}
...
inline TrialOut run_trial(const Config& cfg, uint64_t trial, Ctx& ctx) {
  ...
  for (int attempt = 0; attempt < 8 && !started; attempt++) {
    ...
    if (family == "torus" && P->d >= 4) torus_guess(*P, rng, x); else if (family == "vertical" && P->d >= 3) vertical_guess(*P, rng, x); else random_guess(*P, rng, K0, gamma, x);
    R.reduce(x.data(), y.data());
    double kin, pot; double A = R.f(y.data(), nullptr, ctx.w, &kin, &pot);
    if (!std::isfinite(A) || kin <= 0) continue;
    double lam = optimal_scale(kin, pot, P->alpha); for (double& v : y) v *= lam;
