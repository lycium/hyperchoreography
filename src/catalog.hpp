...
struct RecHdr {                     // fixed-size record header, 152 bytes
  ...
  int32_t N = 0, d = 0, deff = 0, K = 0, M = 0, cover = 1, morse = -1, nullity = -1, hits = 1, nm = 0, symlen = 0, pad = 0;
...
struct Record {
  RecHdr h;
  std::vector<int32_t> modes; std::vector<double> coef, Lsv, pca; std::string sym;
  ...
  bool write(FILE* f) const {
    RecHdr hh = h; hh.nm = (int32_t)modes.size(); hh.symlen = (int32_t)sym.size();
    hh.nbytes = (uint32_t)(sizeof(RecHdr) + modes.size() * 4 + (coef.size() + Lsv.size() + pca.size()) * 8 + sym.size());
    ...
    return std::fwrite(&hh, sizeof hh, 1, f) == 1 && (modes.empty() || std::fwrite(modes.data(), 4, modes.size(), f) == modes.size())
        ...
        && std::fwrite(pca.data(), 8, pca.size(), f) == pca.size() && (sym.empty() || std::fwrite(sym.data(), 1, sym.size(), f) == sym.size());
  }
  bool read(FILE* f) {
    if (std::fread(&h, sizeof h, 1, f) != 1) return false;
    if (h.magic != 0x31434552u || h.nm < 0 || h.d <= 0 || h.symlen < 0) return false;
    size_t nb = sizeof(RecHdr) + (size_t)h.nm * 4 + ((size_t)h.nm * 2 * h.d + 2 * (size_t)h.d) * 8 + h.symlen;
    if (nb != h.nbytes) return false;
    modes.resize(h.nm); coef.resize((size_t)h.nm * 2 * h.d); Lsv.resize(h.d); pca.resize(h.d); sym.resize(h.symlen);
