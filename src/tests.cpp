...
static double circle_radius(int N, double alpha = 1.0) {
...
int main() {
  ...
  { int n = 130; la::Rng rng(5); std::vector<double> A((size_t)n * n), A0;
...
#ifdef HAVE_MPFR
  { mpreal::set_default_prec(400); int N = 3, d = 2; mpreal R = pow(mpreal(1) / sqrt(mpreal(3)), mpreal(1) / 3);
  ...
  { Problem P; P.init(3, 3, 8); la::Rng rng(11); std::vector<double> x, y; random_guess(P, rng, 5, 1.0, x);
    ...
    Problem P2; P2.init(3, 3, 16); std::vector<double> x2(P2.n, 0.0);
  ...
  { double a[4], b[4], c[4]; const int na = ngon_inplane_freq(7, 2, 1.0, a), nb = ngon_inplane_freq(6, 3, 1.0, b), nc = ngon_inplane_freq(8, 0, 1.0, c);
    CHECK(na == 2 && std::fabs(a[0] - 0.501584) < 1e-6 && std::fabs(a[1] - 1.014133) < 1e-6 &&
          ...
          nc == 2 && std::fabs(c[0] + 1) < 1e-12 && std::fabs(c[1] - 1) < 1e-12,
  ...
  {
    const int N = 5, d = 3, K = 12; Problem P; P.init(N, d, K); Work w; w.resize(P); double worst = 0;
    for (int m1 : {1, 2}) {
      std::vector<double> x(P.n, 0.0), H; const double rho = circle_radius(N) * std::pow((double)m1, -2.0 / 3);
      set_mode(P, x, m1, 0, rho, 0.0); set_mode(P, x, m1, 1, 0.0, rho); hessian(P, x.data(), H, w);
      int inv = 1; for (int u = 1; u < N; u++) if ((m1 * u) % N == 1) inv = u;
      for (int mu = 0; mu < P.nm; mu++) { const int m = P.modes[mu], k = (m * inv) % N; const double lam = PI * (m * m - m1 * m1 * std::pow(ngon_vertical_freq(N, k), 2));
        for (int h = 0; h < 2; h++) { const int i = (2 * mu + h) * d + 2;
          for (int j = 0; j < P.n; j++) worst = std::max(worst, std::fabs(H[(size_t)i * P.n + j] - (j == i ? lam : 0.0)) / (1 + std::fabs(lam))); } }
    }
    CHECK(worst < 1e-10, "transverse Hessian of the m1-fold N-gon cover (m1 = 1, 2; N = 5) is diagonal with pi(m^2 - m1^2 w_k^2): %.1e", worst); }

  std::printf("[inertial effective dimension]\n");
  { const int N = 4, d = 4; std::vector<int> md = {1};
    std::vector<double> c(2 * d, 0.0); c[0] = 1.0; c[d + 1] = 1.0;
    std::vector<double> Om((size_t)d * d, 0.0), sv;
    int flat = inertial_deff(N, md, d, c.data(), Om.data(), sv);
    Om[0 * d + 3] = 1.0; Om[3 * d + 0] = -1.0;
  ...
  { const int N = 3, d = 2, K = 8; double R3 = 0; for (int k = 1; k < N; k++) R3 += 1.0 / (4.0 * std::sin(PI * k / N));
    ...
    std::vector<double> x(P.n, 0.0), g(P.n); int mu = 1;
  ...
  { const int N = 5, d = 7, K = 10; Problem P; P.init(N, d, K); la::Rng rng(31);
  ...
  { const int N = 5, d = 7, K = 10; Problem P; P.init(N, d, K); la::Rng rng(7);
  ...
  { const int N = 5, d = 7, K = 10; Problem P; P.init(N, d, K); la::Rng rng(11);
    std::vector<double> x, G; random_guess(P, rng, 5, 1.0, x);
    int g0 = gauge_basis(P, x.data(), G);
    P.set_omega(g2_omega(1, 2)); int g1 = gauge_basis(P, x.data(), G);
    P.set_omega(g2_omega(0, 1)); int g2r = gauge_basis(P, x.data(), G);
  ...
  { la::Rng rng(43); const int d = 7, k = 3; std::vector<int> modes = {1, 2, 3, 5};
    std::vector<double> coef(modes.size() * 2 * d);
    ...
    for (int i = 0; i < ns; i++) { int mu = i < nm ? nm - 1 - i : i - nm, sn = i < nm ? -1 : 1;
      sm[i] = sn * modes[mu];
    ...
    for (int i = 0; i < W.n; i++) { std::complex<double> v = std::complex<double>(0, -1) * Ac[i];
  ...
  { la::Rng rng(11); bool ok = true; std::string line;
    for (int d : {6, 7, 8}) { int k, id; calib_pick(d, k, id);
      ...
      std::vector<double> S((size_t)d * d, 0.0), R, c2(coef.size(), 0.0);
      ...
      std::vector<double> flat(coef.size(), 0.0);
  ...
  { const int N = 5, d = 3, K = 12; Problem P; P.init(N, d, K); Work w; w.resize(P);
    std::vector<double> x(P.n, 0.0); const double R = 0.9;
    x[0 * d + 0] = R; x[1 * d + 1] = R;
  ...
  { const int N = 3, d = 2; std::vector<int> modes = {1, 2, 4, 5, 7, 8};
    std::vector<double> coef(modes.size() * 2 * d, 0.0);
    la::Rng rng(9);
  ...
  { la::Rng rng(17); const int d = 8; std::vector<int> modes = {1, 2, 3, 4, 7};
    std::vector<double> cos_only(modes.size() * 2 * d, 0.0), full(modes.size() * 2 * d);
    for (double& v : full) v = rng.normal();
    for (size_t mu = 0; mu < modes.size(); mu++) for (int a = 0; a < d; a++) cos_only[mu * 2 * d + a] = full[mu * 2 * d + a];
    auto rel = [&](const std::vector<double>& cf, int k) {
    ...
    CHECK(same && err == 0 && dup == 1, "3 records written/read bit-exact (%zu coefficients, extra[] round trips), duplicate lookup -> %ld", cat.recs[0].coef.size(), dup); }
  {
    ...
    for (int k = 0; k < 2; k++) { Record r; std::vector<double> x; random_guess(P, rng, 3, 1.0, x); r.set_solution(P, x.data());
      ...
      if (k) { r.extra[Record::NEX + 1] = 0.75; r.extra[Record::NEX + 2] = -0.75; }
  ...
  { Catalog cat; Problem P; P.init(3, 2, 8); la::Rng rng(9);
    Record a, b; std::vector<double> x, y;
  ...
  { const int N = 4, d = 3, nd = N * d, n2 = 2 * nd; la::Rng rng(3); std::vector<double> Y(n2); for (double& y : Y) y = rng.normal();
    std::vector<std::vector<double>> tr = {{1, 0, 0}}, ro(1, std::vector<double>(9, 0.0)); ro[0][1] = -1; ro[0][3] = 1; ro[0][5] = 2; ro[0][7] = -2;
    auto Q = [&](const std::vector<double>& Z, int r) { double q = 0;
      if (r == 0) { for (int k = 0; k < N; k++) q += Z[nd + k * d]; return q; }
      if (r == 1) return nbody_energy<double>(N, d, 1.0, Z.data(), Z.data() + nd);
      for (int k = 0; k < N; k++) for (int a = 0; a < d; a++) for (int b = 0; b < d; b++) q += ro[0][a * d + b] * Z[k * d + b] * Z[nd + k * d + a]; return q; };
    std::vector<double> J; conserved_jac<double>(N, d, 1.0, Y.data(), tr, ro, J); double err = 0;
    for (int r = 0; r < 3; r++) for (int i = 0; i < n2; i++) { auto Yp = Y, Ym = Y; Yp[i] += 1e-6; Ym[i] -= 1e-6; err = std::max(err, std::fabs((Q(Yp, r) - Q(Ym, r)) / 2e-6 - J[(size_t)r * n2 + i])); }
    CHECK(err < 1e-7, "conservation-law rows (momentum, energy, angular momentum) match central differences to %.1e", err); }
  {
    ...
    { std::vector<double> p0(Z.begin(), Z.begin() + nd), v0(Z.begin() + nd, Z.end());
    ...
    set_d(V.Psi[0], 1.0); set_d(V.Psi[(size_t)n2 + 7], 1.0);
    ...
    CHECK(r2 < 1e-20 && P.ok && P.kappa < 0.1 && P.span && P.nonrigid && mpfr_cmp_d(P.action.lo, 8.12397549) <= 0 && mpfr_cmp_d(P.action.hi, 8.12397550) >= 0,
          "existence proof of the eight: %s, contraction %.1e, closure %.1e, spans R^2 %d, non-rigid %d, action %s", P.ok ? "proven" : P.why.c_str(), P.kappa, P.closure, (int)P.span, (int)P.nonrigid, P.action.str(12).c_str()); }
#endif
